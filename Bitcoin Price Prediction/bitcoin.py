...
def computeDelta(wt, X, Xi):
    ...
    for i in range(len(Xi)):
      xi = Xi.iloc[i]
      sim = np.exp(wt * computeSimilarity(x, xi.iloc[0 : len(xi) - 1]))
      den += sim
      num += xi[-1]*sim
...
def computeSimilarity(a, b):
    ...
    Implemented as per equation 9.
    This is called in the computeDelta function.
    """
    M = len(a)
    a_mean = np.mean(a)
    b_mean = np.mean(b)
    ...
    for z in range(M):
      res += (a.iloc[z] - a_mean)*(b.iloc[z] - b_mean)

    return res/(M * np.std(a) * np.std(b))
